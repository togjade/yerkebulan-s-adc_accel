...
void SOF_Callback(void)
{
	...
	TIM_Cmd(TIM4, DISABLE);
		...
		{
		//UserToPMABufferCopy(((uint8_t *)&USB_buffer), GetEPRxAddr(ENDP1), sendBytes);
		addresBuffer=GetEPTxAddr(ENDP1);
			time_counter=0;
		//	startSample=128;
		}
		else
		{
			//UserToPMABufferCopy(((uint8_t *)&USB_buffer), GetEPTxAddr(ENDP1), sendBytes);
			addresBuffer=GetEPRxAddr(ENDP1);
			//time_counter=64;
			//startSample=0;
		}
		//dataCheck=0;
